$(document).ready(function(){
  ...
  $("#add-card-button").on("click", e => {
     ...
     if(!imagePath) return;
      ...
      createAndInsertCard({cardImageUrl: imagePath, cardTitle: title, cardText: text});
  });
  ...
  $("#cards-container").on("click", ".card .delete-button", (e) => { /* Ovako kad dodamo novu karticu onda mozemo udrit X na njon, jer ka #cards-container uvik postoji u DOM-u */
...
$("#search-box").on("keyup", e => {
   ...
   // Fetch all cards from the DOM
   $("#cards-container > article.card").each(function(index, el) {
       const searchPattern = el.textContent.toLowerCase().indexOf(searchKey.toLowerCase());
       //console.log(index, el);
       if(searchPattern < 0) { // Dodaj hidden ako ga nema
           $(el).addClass("hidden");
       }
       if(searchPattern >= 0 || el.TextContent === "") {
           $(el).removeClass("hidden"); // Makni hidden ako ga ima
       }
   });
